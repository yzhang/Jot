(function() {
  var Expression, Jot, Module, Node, Partial, Plain, Template,
  ...
  Jot.Partial = Partial = (function(_super) {

    __extends(Partial, _super);

    Partial.include(Jot.Helper);

    function Partial(indention, name) {
      this.indention = indention;
      this.name = name;
      this.mode = 'partial';
      this.name = this.name.match(/^\s*([a-zA-Z\{][\{\}a-zA-Z0-9-_]*)\s*$/)[1];
      if (!this.name) throw "Illegal partial name";
    }

    Partial.prototype.render = function(obj) {
      var line, partial, partialName;
      partialName = this.interpolate(this.name, obj);
  ...
  Jot.Template = Template = (function(_super) {
    ...
    function Template(src) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        line = _ref[_i];
        this.isBlank(line) || this.parseLine(line);
      }
    }

    Template.prototype.parseLine = function(line) {
      var content, indention, node, parent, prefix, result;
      result = line.match(/^(\s*)([:=\-\!])?(.*)$/);
      indention = result[1].replace('\t', '  ').length;
      prefix = result[2];
      content = result[3];
      parent = this.indentions[indention - 2];
      if (prefix === '!') return;
      ...
      return this.indentions[indention] = node;
    };
